...
resumen_km <- capture.output(summary(fit_ajcc_km))

# Quitar las líneas del Call
inicio <- which(resumen_km == "")[1]
cat(paste(resumen_km[(inicio + 1):length(resumen_km)], collapse = "\n"))
...
# Mostrar resumen del modelo sin el Call
resumen_km_grade <- capture.output(summary(fit_grade_km))

# Quitar las líneas del Call
inicio_grade <- which(resumen_km_grade == "")[1]
cat(paste(resumen_km_grade[(inicio_grade + 1):length(resumen_km_grade)], collapse = "\n"))
```

La Figura \ref{fig:tumor-grade-kaplan-meier-curve-plot} muestra una asociación significativa entre el grado tumoral y la supervivencia global de los pacientes (log-rank, *p* < 0,0001). Se observó una disminución progresiva de la supervivencia conforme aumentaba el grado tumoral. Los pacientes con tumores G4 presentaron el peor pronóstico, con un marcado descenso de la probabilidad de supervivencia durante los primeros años de seguimiento y valores inferiores al 25 % a partir de los 60-72 meses (5-6 años). Por el contrario, los pacientes con tumores G2 mostraron las mayores probabilidades de supervivencia entre los grupos con un tamaño muestral representativo, manteniendo una supervivencia cercana al 75 % durante aproximadamente los primeros 72 meses (6 años) y superior al 50 % en los últimos periodos de seguimiento. Los tumores G3 presentaron una evolución intermedia, con una disminución progresiva de la supervivencia que los situó entre los grupos G2 y G4 a lo largo de prácticamente todo el seguimiento.

Asimismo, las curvas de supervivencia mostraron una separación temprana entre los distintos grados tumorales, especialmente entre los grupos G2 y G4, lo que sugiere que el grado histológico influye de forma relevante en el pronóstico desde las primeras etapas del seguimiento. Aunque los pacientes clasificados como G1 mantuvieron una supervivencia del 100 % durante todo el estudio, este resultado debe interpretarse con cautela debido al reducido número de casos incluidos en este grupo (*n* = 14).

En conjunto, estos resultados indican que un mayor grado tumoral se asocia con una menor supervivencia global y un peor pronóstico clínico, reforzando el valor del grado histológico como factor pronóstico en esta cohorte.
...
# Mostrar resumen del modelo sin el Call
resumen_km_age <- capture.output(summary(fit_age_km))

# Quitar las líneas del Call
inicio_age <- which(resumen_km_age == "")[1]
cat(paste(resumen_km_age[(inicio_age + 1):length(resumen_km_age)], collapse = "\n"))
...
ggsurvplot(fit_age_km,
           ...
           legend.labs = c("<50 años", "50-70 años", ">70 años"))
```

La Figura \ref{fig:age-kaplan-meier-curve} muestra diferencias significativas en la supervivencia global entre los distintos grupos de edad al diagnóstico (log-rank, *p* < 0,0001). Los pacientes menores de 50 años presentaron las mayores probabilidades de supervivencia durante todo el periodo de seguimiento, manteniendo valores superiores al 75 % durante aproximadamente los primeros 60-72 meses (5-6 años) y cercanos al 70 % en los últimos periodos evaluables.

Por su parte, los pacientes con edades comprendidas entre 50 y 70 años mostraron la supervivencia más desfavorable, con un descenso progresivo de la probabilidad de supervivencia hasta situarse por debajo del 50 % alrededor de los 72 meses (6 años) y en torno al 25 % a partir de los 96-108 meses (8-9 años). Los pacientes mayores de 70 años presentaron una supervivencia intermedia, superior a la observada en el grupo de 50-70 años durante gran parte del seguimiento y cercana al 50 % en los periodos finales.

En conjunto, estos resultados indican que la edad al diagnóstico se asocia significativamente con la supervivencia global de la cohorte. No obstante, el patrón observado no muestra una disminución lineal de la supervivencia con la edad, ya que el grupo de 50-70 años presentó una supervivencia inferior a la observada en los pacientes mayores de 70 años.
