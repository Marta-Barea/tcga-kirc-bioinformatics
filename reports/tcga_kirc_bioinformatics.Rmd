...
## Acceso y exploración de los datos TCGA-KIRC
...
Para obtener estos datos se utilizó el paquete **TCGAbiolinks**. En concreto, `GDCquery()` se empleó para definir la búsqueda de los archivos transcriptómicos del proyecto, `GDCdownload()` para descargarlos en local y `GDCquery_clinic()` para recuperar la información clínica de los pacientes. Con estas funciones fue posible reunir, de forma ordenada, los datos que posteriormente se utilizarán en los análisis.

La parte de descarga se trasladó a un script externo, `scripts/download_tcga_kirc_data.R`, para mantener separado el proceso de adquisición de datos del proceso de análisis. Esta decisión resulta útil porque evita que, cada vez que se renderiza el documento, se vuelva a lanzar la descarga desde GDC, algo que puede ralentizar bastante la compilación del informe.

El script se ejecuta una única vez desde la raíz del proyecto con el siguiente comando:

```{r, eval=FALSE}
system("Rscript scripts/download_tcga_kirc_data.R")
```

A continuación se muestra el contenido del script. Se incluye aquí únicamente con fines documentales, pero no se ejecuta dentro del propio RMarkdown para evitar que el renderizado del informe vuelva a iniciar la descarga.
...
Una vez ejecutado el script, los archivos quedan almacenados en la carpeta **data** del repositorio. A partir de ese momento, ya no es necesario volver a ejecutar la descarga para esta parte del documento. En su lugar, puede mostrarse directamente qué contenido se ha guardado en disco, cuántos archivos se han obtenido, en qué formato están y cuánto espacio ocupan.
