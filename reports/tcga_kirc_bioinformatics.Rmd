...
## Bases biológicas y relevancia cínica del cáncer
...
```{r continuous-descriptive-plots-figure, fig.width=6.5, fig.height=5.5, fig.align='center', out.width='90%', fig.pos='H', fig.cap='Distribución de las variables continuas del conjunto de datos clínico TCGA-KIRC representada mediante diagrama de violín.'}
# Figura combinada
(p1 | p2) /
...
```{r global-kaplan-meier-curve, fig.width=7.5, fig.height=4.2, fig.align='center', out.width='90%', fig.pos='H', fig.cap='Curva de Kaplan-Meier que muestra la supervivencia global de la cohorte TCGA-KIRC.'}
# Cargar librerías necesarias
library(survival)
library(survminer)
...
```{r ajcc-kaplan-meier-curve, fig.width=7.5, fig.height=8.2, fig.align='center', out.width='90%', fig.pos='H', fig.cap='Curvas de Kaplan-Meier estratificadas por estadio patológico AJCC, con intervalos de confianza y tabla de riesgo.'}
# Cargar librerías necesarias
library(survival)
library(viridis)
library(survminer)
...
```{r tumor-grade-kaplan-meier-curve-plot, fig.width=7.5, fig.height=8.2, fig.align='center', out.width='90%', fig.pos='H', fig.cap='Curvas de Kaplan-Meier estratificadas por grado tumoral, con intervalos de confianza y tabla de riesgo.'}
...
```{r age-kaplan-meier-curve, fig.width=7.5, fig.height=8.2, fig.align='center', out.width='90%', fig.pos='H', fig.cap='Curvas de Kaplan-Meier estratificadas por grupos de edad al diagnóstico, con intervalos de confianza y tabla de riesgo.'}
# Cargar librerías necesarias
library(survival)
library(survminer)
library(dplyr)
...
```{r sex-kaplan-meier-curve-plot, fig.width=7.5, fig.height=8.2, fig.align='center', out.width='90%', fig.pos='H', fig.cap='Curvas de Kaplan-Meier estratificadas por sexo, con intervalos de confianza y tabla de riesgo.'}
# Representar la curva de Kaplan-Meier
...
Por este motivo, se realizó una conversión de los identificadores `Ensembl` a símbolos génicos utilizando el paquete `org.Hs.eg.db` de R/Bioconductor, que contiene anotaciones actualizadas para genes humanos, junto con el paquete `AnnotationDbi`, que proporciona las funciones necesarias para realizar la correspondencia entre diferentes sistemas de identificación génica. 
...
```{r convert-gene-identifiers}
# Cargar librerías necesarias
library(org.Hs.eg.db)
library(AnnotationDbi)

# Eliminar el sufijo de versión de los identificadores Ensembl
ensembl_ids <- sub("\\..*", "", counts_df$gene_id)

# Convertir identificadores Ensembl a símbolos génicos
gene_symbols <- mapIds(
  org.Hs.eg.db,
  keys = ensembl_ids,
  column = "SYMBOL",
...
# Añadir los símbolos génicos al data frame
counts_df$gene_symbol <- gene_symbols

# Visualizar los primeros resultados
head(
  counts_df[, c("gene_id", "gene_symbol")]
)
```

Tras esta conversión, cada fila del conjunto de datos dispone tanto de su identificador Ensembl original como de su correspondiente símbolo génico, facilitando la interpretación biológica de los análisis posteriores y la presentación de resultados.
